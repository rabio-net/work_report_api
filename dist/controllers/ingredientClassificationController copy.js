...
class IngredientClassificationController {
    static async getIngredientClassificationAll(request, response) {
        const pwAuth = (0, token_1.getPwAuthToken)(request);
        if (!pwAuth) {
            response.json(JSON.stringify({ status: 401 }));
            return;
        }
        const res = await new ingredientClassificationService_1.default().getIngredientClassificationAll(pwAuth);
        response.json(JSON.stringify(res));
    }
}
exports.default = IngredientClassificationController;
//# sourceMappingURL=ingredientClassificationController%20copy.js.map